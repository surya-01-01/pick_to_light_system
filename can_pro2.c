 ...
 * Author: surya
 *
 * Created on 8 May, 2025, 11:20 AM
 */

#include "pick2light.h"
/* Global Variables */
extern unsigned char trans_buffer[8];
...
void can_transmit(void)
{
	...
	TXB0D0 = trans_buffer[0];			/* DataByte 0 */
	TXB0D1 = trans_buffer[1];			/* DataByte 1 */
	TXB0D2 = trans_buffer[2];			/* DataByte 2 */
	TXB0D3 = trans_buffer[3];			/* DataByte 3 */
	TXB0D4 = trans_buffer[4];			/* DataByte 4 */
	TXB0D5 = trans_buffer[5];			/* DataByte 5 */
	TXB0D6 = trans_buffer[6];			/* DataByte 6 */
	TXB0D7 = trans_buffer[7];			/* DataByte 7 */
